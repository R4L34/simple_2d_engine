...
public class Entity {
	...
	public <T extends Component> T GetComponentByGroup(String group, Class<T> type) {
	...
	// By group (single group)
	public <T extends Component> List<T> GetComponentsByGroup(String group, Class<T> type) {
		List<T> results = new ArrayList<>();
		for (Component c : components) {
			for (String g : c.getGroups()) {
				if (g.equals(group) && type.isInstance(c)) {
	...
	public boolean hasDependencies(List<Class<? extends Component>> dependencies) {
		int count = 0;
		for (Class<? extends Component> dependency : dependencies) {
			for(Component c : components) {
				if(dependency.isInstance(c)) {
					count++;
					break;
				}
			}
		...
		//System.out.println("The Entity " + name + " does not have neseccary dependencies");
		return false;
	}
